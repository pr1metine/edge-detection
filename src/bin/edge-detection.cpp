...
int sc_main(int argc, char *argv[]) {
  ...
  convolution::Layer<char> blur_layer("blur_layer", reader.get_output_height(),
                                      reader.get_output_width(),
                                      convolution::ConvolutionType::BLUR, 0);
  convolution::Layer<char> convolution_layer(
      "convolution_layer", blur_layer.get_output_height(),
      blur_layer.get_output_width(),
      convolution::ConvolutionType::SOBER_DIAGONAL, 0);
  ...
  for (size_t i = 0; i < reader_convolution_chan.size(); ++i) {
    ...
    blur_layer.input_matrix(reader_convolution_chan[i]);
  }

  sc_core::sc_vector<sc_core::sc_signal<char>> convolution_blur_chan;
  convolution_blur_chan.init(blur_layer.get_output_height() *
                             blur_layer.get_output_width());
  for (size_t i = 0; i < convolution_blur_chan.size(); ++i) {
    blur_layer.output_matrix(convolution_blur_chan[i]);
    convolution_layer.input_matrix(convolution_blur_chan[i]);
  }

  sc_core::sc_vector<sc_core::sc_signal<char>> blur_writer_chan;
  blur_writer_chan.init(convolution_layer.get_output_height() *
                        convolution_layer.get_output_width());
  for (size_t i = 0; i < blur_writer_chan.size(); ++i) {
    convolution_layer.output_matrix(blur_writer_chan[i]);
